...
void Check_for_error(int local_ok, char fname[], char message[], 
      MPI_Comm comm);
void Get_dims(int* padded, int* local_padded, int* m_p, int* local_m_p, int* n_p, int* local_n_p,
      int my_rank, int comm_sz, MPI_Comm comm);
void Allocate_arrays(double** local_A_pp, double** local_x_pp, 
      double** local_y_pp, int local_m, int n, int local_n, 
      MPI_Comm comm);
void Read_matrix(char prompt[], int padded, int local_padded, double local_A[], int m, int local_m, 
      int n, int my_rank, MPI_Comm comm);
void Read_vector(char prompt[], int padded, int local_padded, double local_vec[], int n, int local_n, 
...
int main(void) {
   ...
   Get_dims(&padded, &local_padded, &m, &local_m, &n, &local_n, my_rank, comm_sz, comm);
   Allocate_arrays(&local_A, &local_x, &local_y, local_m, n, local_n, comm);
   Read_matrix("A", padded, local_padded, local_A, m, local_m, n, my_rank, comm);

   Print_matrix("A", local_A, m, local_m, n, my_rank, comm);

   Read_vector("x", padded, local_padded, local_x, n, local_n, my_rank, comm);
...
void Get_dims(
   ...
   MPI_Bcast(padded, 1, MPI_INT, 0, comm);
